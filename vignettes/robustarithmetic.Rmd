...
vignette: >
  %\VignetteIndexEntry{Verified interval arithmetic with correctly rounded kernels}
...
The **fast** level evaluates with the correctly rounded binary64 implementation included in
the package: fifteen CORE-MATH kernels and the hardware square root. It widens each finite
endpoint by the pre-registered slack of two outward steps. The **rigorous** level evaluates
over `Rmpfr` with a directed-rounding bridge. When a verdict would otherwise fall inside the
slack — that is, when the fast level cannot decide the question being asked — the package
escalates through a ladder of precisions rather than returning a confident answer it cannot
defend.
...
The slack is not a tuning knob chosen for convenience. The pre-registered formula is
`ceiling(2 * e + 1)`, where correct rounding gives the declared bound `e = 0.5`; the same
formula therefore gives two steps for all sixteen functions. `ra_measure_library_error()`
checks the included route against an independent multiprecision referent. The fast result
still carries `measured` provenance because correct rounding of the included software has
been verified numerically but is not established here as a theorem for every kernel.

The included evaluator also verifies itself at namespace load against independent difficult
cases, bit for bit. A healthy session starts silently. `ra_fast_level_status()` exposes that
verification and the first-use multiprecision audit. If either check degrades the fast level,
the first affected result emits one typed warning and escalates to the rigorous level; later
results in that session remain rigorous without repeating the warning. Without `Rmpfr`, an
affected evaluation refuses because no rigorous result is available to replace it. This rule
is identical whether the package is attached with `library()` or called with `::`.

```{r}
ra_fast_level_status()$degraded
```
